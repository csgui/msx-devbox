...
RUN apt-get update && apt-get install -y software-properties-common \
    ...
    && apt-get clean && rm -rf /var/lib/apt/lists/*
# Clone and build openMSX 21 with binaries in /usr/local/bin
...
# Copy openMSX binaries from Ubuntu builder - need BOTH /usr/local AND /opt
COPY --from=openmsx-builder /usr/local /usr/local
COPY --from=openmsx-builder /opt/openMSX /opt/openMSX
# Copy the newer libstdc++ from Ubuntu (for GLIBCXX_3.4.31 and 3.4.32)
...
# Install runtime dependencies - INCLUDING openMSX's shared library dependencies
RUN apt-get update && apt-get install -y \
